...
t = np.linspace(1, len(an1), len(an1))
...
loc = []
it = 0
for i in range(0, len(an1), 12):
		loc.append(i)
		anios.append(str(1979+it))
		it+=1

#Se generan las graficas de las anomalias y el SOI en funcion del tiempo. Se realizaron en la misma figura pero con dos ejes verticales distintos, para visualizar mejor las posibles correlaciones.
plt.title(r"$Anomal\'ias$ $de$ $temperatura$ $y$ $SOI$ $en$ $funci\'on$ $del$ $tiempo$")
plt.plot(t[:-1], indices[1236:], label=r"$SOI$", color='orange')
plt.ylabel(r"$SOI$", fontsize=16)
plt.xlabel(r"$Tiempo$ $(meses$ $por$ $a\~no)$")
plt.xticks(loc, anios, size='small', rotation=90)
plt.legend(loc=0)
plt.twinx()
plt.plot(t, an1, label=r"$130E$-$80W$", color='r')
plt.plot(t, an2, label=r"$160E$-$80W$", c='g')
plt.plot(t, an3, label=r"$180W$-$100W$", color='b')
plt.minorticks_on()
...
#Se guardan las variables relevantes (el SOI y las tres anomalias de temperatura). Solo se toman los datos para los anios de 1979 en adelante y no se tiene en cuenta el dato para el segundo mes del anio 2017.
